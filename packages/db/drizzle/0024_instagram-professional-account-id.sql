ALTER TABLE "instagram_accounts" ADD COLUMN "instagram_professional_account_id" text;--> statement-breakpoint
CREATE UNIQUE INDEX "instagram_accounts_professional_account_uq" ON "instagram_accounts" USING btree ("instagram_professional_account_id") WHERE "instagram_accounts"."instagram_professional_account_id" is not null;--> statement-breakpoint
ALTER TABLE "instagram_accounts" ADD CONSTRAINT "instagram_accounts_professional_account_id_numeric" CHECK ("instagram_accounts"."instagram_professional_account_id" is null or "instagram_accounts"."instagram_professional_account_id" ~ '^[0-9]{1,80}$');
...
AS $$
  ...
  WHERE target_account_id ~ '^[0-9]{1,80}$'
    ...
    AND (
      account.instagram_account_id = target_account_id
      OR account.instagram_professional_account_id = target_account_id
    )
  ORDER BY (account.instagram_account_id = target_account_id) DESC
  LIMIT 1
$$;
--> statement-breakpoint
REVOKE ALL ON FUNCTION app_resolve_instagram_tenant(text) FROM PUBLIC;
--> statement-breakpoint
DO $$
BEGIN

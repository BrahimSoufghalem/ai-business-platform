...
نتيجة `handoff` لا تبقى إشارة داخل AI Run فقط. ينفذ Conversation service أمرًا idempotent ينشئ سجلًا منظمًا، يبني ملخصًا منقحًا، يرسل إشعارًا واحدًا للعميل، وينقل المحادثة ذريًا إلى `needs_human`. الاستلام يربط المحادثة والتحويل بالمستخدم الحالي؛ التحرير يعيدهما إلى الطابور؛ العودة للبوت أو الإغلاق تحل التحويل. تمنع حواجز الكتابة رد البوت والموظف بالتوازي. التفاصيل في [Human Handoff](HUMAN_HANDOFF.md).
